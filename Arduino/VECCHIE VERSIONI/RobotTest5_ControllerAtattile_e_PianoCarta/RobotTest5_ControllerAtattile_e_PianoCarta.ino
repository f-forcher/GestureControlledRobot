...
void loop() {
  ...
  if(BlueToothSerial.available()){//check if there's any data sent from the remote bluetooth shield
    ...
    //Attiva la modalità pianocarta, altoparlante sul pin 10
    case 'p':
     ...
     //Gear, in realtà fa in modo che il dritto sia il dietro (diverso da retromarcia)
     case 'g':{
      if(!isRETRO){
        isRETRO=false;
      }
